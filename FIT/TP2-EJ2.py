...
#Datos almacenados en vectores de cada planeta

Mp=dat[:,0]                 #Masa final de cada planeta [kg]
Rp=dat[:,1]                 #Radio final de cada planeta [m]
...
t=np.arange(0,T_ac,dt)*1e-6 #Vector de tiempo [Ma] parto en t=0 por con. inicial

#Defino las funciones a resolver (EDOs)
...
def f(R,Rp,M,Mp,T,T0,ap,dap,rhop):
    ...
    #Defino las funciones para el incremento en el método de Euler
    dR=ctte1*(1-(R/Rp)**3)
    dM=ctte2*(Mp-M)*R**2
...
for i in range(len(dat[:,0])):
    ...
    for j in range(Nit-1): #Debe ser Nit-1 porque hemos colocado el valor en la posición [0]
        ...
        #Euler Yi+1 = yi+h*f(xi,yi)   h-> Paso, f(xi,yi) función a evaluar
        # h*f(xi,yi) -> Incremento con el cual me acerco a la solución
...
Av=R[:,0];Bv=M[:,0];Cv=T[:,0]  #Datos Venus
At=R[:,1];Bt=M[:,1];Ct=T[:,1]  #Datos La Tierra
Am=R[:,2];Bm=M[:,2];Cm=T[:,2]  #Datos Marte
